...
@NamedQueries({
	...
	@NamedQuery(name="findAvailable", query="select d from Domain d where d.numberTimesRequested = :numberTimesRequested")
})
public class Domain {
	@Column(name="Id")
	@Id()
	...
	public static List<Domain> findAvailable(EntityManager mgr) {
		Query query = mgr.createNamedQuery("findAvailable");
		query.setParameter("numberTimesRequested", 0);
